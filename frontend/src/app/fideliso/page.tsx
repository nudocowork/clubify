...
/**
 * Landing de Fideliso = CLON 1:1 de la de Clubify (soyclubify.com): misma
 * estructura, jerarquía, bloques, espaciados y tamaños. Reusa los MISMOS
 * componentes (HeroTrio, FidelizacionBanner, HeroBanner, InfoLinksBanner,
 * LandingPricingCheckout). Lo único que cambia:
 *   - Logo → logo de la marca (R2 por host) con fallback estático.
 *   - Verde (tokens brand/ok) → AZUL #2563EB, vía override CSS scopeado
 *     .fideliso-theme (mismo patrón que .sellea-theme → naranja).
 *   - Textos → Fideliso.
 * Preview: /fideliso · Producción: www.fideliso.com (middleware reescribe
 * la raíz del host de la marca a /fideliso).
 */
export const metadata: Metadata = {
  title: 'Fideliso · Fidelización digital para tu negocio',
  description:
    'Tarjetas de sellos en Apple y Google Wallet, menús con IA, pedidos por WhatsApp e InfoLinks. Cada compra deja su sello.',
};

const LOGOS = [
  'Café del Día', 'Burger Lab', 'Bowls & Co', 'Pizza Roma',
  'Valmont Barbería', 'Panadería 21', 'Sushi Kira', 'Helados Tina', 'Wok Explosivo',
];
...
const FAQS = [
  { q: '¿Cuánto pago y en qué moneda?', a: 'Desde USD 68/mes en el plan mensual. También tenemos planes Trimestral, Semestral y Anual con descuento por compromiso. Te mostramos el equivalente en tu moneda local al cambio del día. Sin contratos — cancelas cuando quieras desde tu panel.' },
...
// config de la marca (WhiteLabel.demoButtonWhatsApp) — se resuelve en runtime.
const WA_TEXT = encodeURIComponent('Hola, quiero saber más de Fideliso');
const WA_LINK_FALLBACK = 'https://wa.me/?text=' + WA_TEXT;
const DEMO_LINK_FALLBACK = 'https://wa.me/';
const igLink = 'https://www.instagram.com/fideliso';
const mailLink = 'mailto:hola@fideliso.com';

// Override CSS scopeado: voltea los tokens verdes (brand/ok) a AZUL dentro de
// .fideliso-theme. Recolorea TODO el árbol (markup propio + componentes
// reusados) sin tocar la estructura. Azul Fideliso #2563EB (400 #3B82F6 / 700 #1D4ED8).
const FIDELISO_THEME_CSS = `
.fideliso-theme .text-brand,.fideliso-theme .hover\\:text-brand:hover{color:#2563EB!important}
.fideliso-theme [class*="bg-brand"]:not([class*="bg-brand-soft"]){background-color:#2563EB!important}
.fideliso-theme [class*="bg-brand-soft"]{background-color:#DBEAFE!important}
.fideliso-theme [class*="border-brand"]{border-color:#2563EB!important}
.fideliso-theme .text-ok,.fideliso-theme [class*="text-ok"]{color:#2563EB!important}
.fideliso-theme [class*="from-brand"]{--tw-gradient-from:#3B82F6 var(--tw-gradient-from-position)!important;--tw-gradient-to:rgb(59 130 246 / 0) var(--tw-gradient-to-position)!important;--tw-gradient-stops:var(--tw-gradient-from),var(--tw-gradient-via),var(--tw-gradient-to)!important}
.fideliso-theme [class*="via-brand"]{--tw-gradient-to:rgb(37 99 235 / 0) var(--tw-gradient-to-position)!important;--tw-gradient-stops:var(--tw-gradient-from),#2563EB var(--tw-gradient-via-position),var(--tw-gradient-to)!important}
.fideliso-theme [class*="to-brand"]{--tw-gradient-to:#1D4ED8 var(--tw-gradient-to-position)!important}
`;
 ...
 *  /fideliso) o sin match, devuelve nulls → la landing usa sus fallbacks. */
...
/** Planes de Fideliso desde su config de pasarela (links por marca). Devuelve
 *  null si el host no matchea (preview /fideliso) → la landing usa su fallback. */
...
export default async function FidelisoLandingPage() {
  const host = headers().get('host') ?? '';
  const [brand, fetchedPlans] = await Promise.all([
    fetchBrandByHost(host),
    fetchPaymentPlans(host),
  ]);
  // Logo: el subido en la marca (R2 por host). Fallback estático en preview.
  const brandLogo = brand.logoUrl ?? '/fideliso-logo.jpg';
  ...
  // Planes: si la marca tiene links configurados (Master Admin → Pasarela de
  // pago), se usan esos. Sino, fallback Mensual $68 / Anual $499 con
  // checkoutUrl null (botón "Próximamente") mientras no haya links cargados.
  const landingPlans: LandingPlan[] = fetchedPlans ?? [
    { id: 'mensual', name: 'Mensual', shortName: 'Mensual', months: 1, price: 68, checkoutUrl: null, description: '' },
    { id: 'anual', name: 'Anual', shortName: 'Anual', months: 12, price: 499, checkoutUrl: null, description: '' },
  ];
  ...
  // FAQ de precios DINÁMICA — usa los mismos valores que el checkout.
  const mensualP = landingPlans.find((p) => p.id === 'mensual')?.price ?? 68;
  ...
  return (
    <main className="fideliso-theme min-h-screen bg-white text-ink">
      <style dangerouslySetInnerHTML={{ __html: FIDELISO_THEME_CSS }} />

      {/* ─────────── Header ─────────── */}
      <header className="sticky top-0 z-30 backdrop-blur-md bg-white/85 border-b border-line/80">
        <div className="mx-auto max-w-7xl px-6 flex items-center justify-between h-16">
          <Link href="/" className="flex items-center" aria-label="Fideliso">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={brandLogo} alt="Fideliso" className="h-11 w-auto max-w-[180px] object-contain" />
          </Link>
      ...
      <section className="relative overflow-hidden">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 70% 60% at 30% 20%, rgba(37,99,235,0.16), transparent 60%), radial-gradient(ellipse 60% 50% at 80% 30%, rgba(29,78,216,0.10), transparent 60%)' }}
        />
        <div className="mx-auto max-w-7xl px-6 pt-8 pb-20 lg:pt-12 lg:pb-28">
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] gap-12 lg:gap-16 items-center">
            ...
            {/* Visual genérico de Clubify: trío de iPhones. */}
      ...
      <section className="border-y border-line/80 bg-bg2/40 py-8 overflow-hidden">
        <div className="mx-auto max-w-7xl px-6">
          <div className="text-center text-[11px] uppercase tracking-[0.18em] text-mute font-semibold mb-5">
            Negocios LATAM creciendo con Fideliso
          </div>
        </div>
      ...
      {/* demos={[]}: NO mostrar los iPhones con menús de clientes de Clubify
          (soyclubify.com/m/...) en el dominio de Fideliso — fuga de marca. */}
      ...
      <footer className="border-t border-line bg-white">
        <div className="mx-auto max-w-7xl px-6 py-14">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 text-sm">
            <div className="col-span-2">
              <div className="flex items-center mb-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={brandLogo} alt="Fideliso" className="h-10 w-auto max-w-[160px] object-contain" />
              </div>
          ...
          <div className="border-t border-line mt-10 pt-6 flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-mute">
            <div>© 2025 Fideliso</div>
            <div className="flex items-center gap-3"><span>www.fideliso.com · @fideliso</span></div>
          </div>
        </div>
      </footer>
    </main>
