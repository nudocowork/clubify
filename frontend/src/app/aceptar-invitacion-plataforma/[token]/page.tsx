...
type Lookup = { email: string; fullName: string; expiresAt: string };
...
export default function AcceptInvitePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${API}/api/superadmin-public/owner-invites/${token}`);
        if (!res.ok) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setState('saving');
    try {
      const res = await fetch(`${API}/api/superadmin-public/owner-invites/${token}/accept`, {
        method: 'POST',
    ...
    >
      ...
      >
        <div
          ...
          style={{ background: 'linear-gradient(135deg, #15803d, #10b981)' }}
        >
          <div
            className="text-[11px] font-bold uppercase opacity-90"
            ...
            <form onSubmit={submit}>
              ...
              <button
                ...
                style={{
                  padding: '12px 16px',
                  borderRadius: 10,
                  background: state === 'saving' || password.length < 8 || password !== confirm ? '#cbd5d2' : '#15803d',
                  color: 'white',
                  border: 'none',
