'use client';
/**
 * Vista pública: slide deck interactivo.
 *
 * URL: /industria/{industrySlug}/{presentationSlug}
 *
 * Features:
 * - Navegación: keyboard (← →, Esc), swipe horizontal mobile, click
 *   en dots/flechas. Esc cierra el deck (vuelve a /industria/{slug}).
 * - 10 layouts (COVER, IMAGE_LEFT, IMAGE_RIGHT, FULL_IMAGE, TEXT_ONLY,
 *   QUOTE, CTA, STATS, COMPARISON, VIDEO) renderizados según enum.
 * - 5 animations (NONE, FADE, SLIDE_RIGHT, SLIDE_UP, ZOOM) — re-disparan
 *   al cambiar de slide vía key={index} en el contenedor.
 * - Mobile first — un solo column en mobile, layouts split (IMAGE_LEFT/
 *   RIGHT, COMPARISON) hacen stack en mobile.
 * - Progress bar arriba.
 * - Dots indicators clickeable abajo (desktop) — en mobile se ocultan.
 */
...
export default function SlideDeckPage() {
  ...
  // Swipe state — usamos refs para no re-renderizar en cada touchmove.
  ...
  const current = deck?.slides[idx] ?? null;
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key === 'ArrowRight' || e.key === ' ') {
        ...
        router.push(`/industria/${slug}`);
      }
    }
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [goNext, goPrev, router, slug]);
  ...
  function onTouchEnd(e: React.TouchEvent) {
    ...
    // Mínimo 50px horizontal Y más horizontal que vertical (sino es scroll).
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg p-4">
        <div className="card card-pad text-center max-w-md">
          ...
          <div className="font-semibold mb-1">Esta presentación todavía no tiene slides</div>
          <Link
            href={`/industria/${slug}`}
            className="btn-primary inline-block mt-4"
          >
  ...
  const accent =
    current.bgColor ?? deck.themeColor ?? deck.industry.themeColor ?? '#0A0A0A';
  ...
  return (
    ...
    >
      ...
      {/* Slide content — key={idx} re-monta para re-disparar la animación */}
      <div
        key={`${current.id}-${idx}`}
      ...
      <style jsx global>{`
        ...
        .animate-deck-fade {
          animation: deck-fade 400ms ease-out;
        }
        .animate-deck-slide-right {
          animation: deck-slide-right 450ms ease-out;
        }
        .animate-deck-slide-up {
          animation: deck-slide-up 450ms ease-out;
        }
        .animate-deck-zoom {
          animation: deck-zoom 500ms ease-out;
        }
      `}</style>
    </div>
  );
...
/** Render del slide según el layout — un componente por enum value. */
function SlideRenderer({
  ...
  if (layout === 'IMAGE_LEFT')
    return <SlideImageSide slide={slide} side="left" textColor={textColor} />;
  if (layout === 'IMAGE_RIGHT')
    return <SlideImageSide slide={slide} side="right" textColor={textColor} />;
  if (layout === 'FULL_IMAGE') return <SlideFullImage slide={slide} />;
  if (layout === 'TEXT_ONLY')
    return <SlideTextOnly slide={slide} textColor={textColor} />;
  ...
  if (layout === 'COMPARISON')
    return <SlideComparison slide={slide} textColor={textColor} />;
    ...
    <div className="max-w-3xl text-center">
      {slide.imageUrl && (
        ...
        <img
          src={slide.imageUrl}
          alt=""
          className="max-h-[40vh] mx-auto mb-6 object-contain"
        />
      )}
      {slide.subtitle && (
        <div className="text-sm md:text-base opacity-80 uppercase tracking-widest mb-2 md:mb-4">
          {slide.subtitle}
        </div>
      )}
      {slide.title && (
      ...
      {slide.body && (
        <p className="mt-4 md:mt-6 text-base md:text-lg opacity-90 leading-relaxed max-w-2xl mx-auto whitespace-pre-line">
          {slide.body}
        </p>
      )}
      {slide.ctaText && slide.ctaUrl && (
        <CtaButton slide={slide} textColor={textColor} />
      )}
    </div>
  );
}

function SlideImageSide({
  slide,
  side,
  textColor,
}: {
  ...
  return (
    <div
      className={`grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 items-center max-w-6xl w-full ${
    ...
    >
      ...
      <div>
        ...
        {slide.body && (
          <p className="mt-4 text-base md:text-lg opacity-90 leading-relaxed whitespace-pre-line">
            {slide.body}
          </p>
        )}
        {slide.ctaText && slide.ctaUrl && (
      ...
      {(slide.title || slide.subtitle || slide.body) && (
        <div className="absolute inset-0 bg-black/40 flex items-end p-6 md:p-12">
          <div className="text-white max-w-3xl">
            ...
            {slide.body && (
              <p className="mt-3 text-base md:text-lg opacity-90 leading-relaxed whitespace-pre-line">
                {slide.body}
              </p>
            )}
          </div>
...
function SlideTextOnly({
  ...
  return (
    <div className="max-w-3xl text-center">
      {slide.subtitle && (
        <div className="text-sm md:text-base opacity-80 uppercase tracking-widest mb-3">
          {slide.subtitle}
        </div>
      )}
      {slide.title && (
      ...
      {slide.body && (
        <p className="text-lg md:text-2xl opacity-90 leading-relaxed whitespace-pre-line">
          {slide.body}
        </p>
      )}
      {slide.ctaText && slide.ctaUrl && (
...
function SlideQuote({
  ...
  return (
    <div className="max-w-3xl text-center">
      ...
      {slide.body && (
        <p className="text-2xl md:text-4xl font-medium leading-snug mt-2 md:mt-4 whitespace-pre-line">
          {slide.body}
        </p>
      )}
      {slide.title && (
        <div className="mt-6 md:mt-8 text-sm md:text-base opacity-80 uppercase tracking-widest">
          — {slide.title}
        </div>
      )}
      {slide.subtitle && (
...
function SlideCta({ slide, textColor }: { slide: Slide; textColor: string }) {
  return (
    <div className="max-w-2xl text-center">
      ...
      {slide.ctaText && slide.ctaUrl && (
        <CtaButton slide={slide} textColor={textColor} prominent />
      )}
    </div>
  );
}
...
function SlideStats({
  ...
  return (
    <div className="max-w-5xl w-full text-center">
      {slide.title && (
        <h2 className="text-2xl md:text-4xl font-bold mb-8 md:mb-12 leading-tight">
          {slide.title}
        </h2>
      )}
      <div
      ...
      >
        {stats.map((s, i) => (
          <div key={i}>
            ...
            <div className="text-sm md:text-base opacity-80 mt-2 md:mt-3 uppercase tracking-wider">
              {s.label ?? ''}
            </div>
          </div>
        ))}
...
function SlideComparison({
  ...
  return (
    <div className="max-w-5xl w-full">
      {slide.title && (
        <h2 className="text-2xl md:text-4xl font-bold mb-6 md:mb-10 text-center leading-tight">
          {slide.title}
        </h2>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white/20 rounded-2xl p-5 md:p-7">
          <div className="text-sm uppercase tracking-widest opacity-90 mb-3 font-semibold">
            {right.title}
          </div>
          <ul className="space-y-2">
...
function SlideVideo({
  ...
  const embedUrl = isYouTube
    ? url.replace(/youtu\.be\/([^?]+)/, 'www.youtube.com/embed/$1')
    : url;
  return (
    <div className="max-w-5xl w-full">
      {slide.title && (
      ...
      {slide.body && (
        <p className="mt-4 text-sm md:text-base opacity-90 text-center whitespace-pre-line">
          {slide.body}
        </p>
      )}
      {slide.ctaText && slide.ctaUrl && (
...
/** Devuelve negro o blanco según el color de fondo — heurística simple
 *  YIQ. Útil cuando el slide no setea textColor explícito. */
