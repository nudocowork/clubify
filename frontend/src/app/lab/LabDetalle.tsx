'use client';
/**
 * Detalle de una propuesta del Lab. Lo montan `/lab/[id]` (página propia) y
 * `/admin/lab/[id]`: el administrador de una marca blanca tiene su Lab dentro
 * del panel y no debe salir a la página suelta para votar o comentar.
 *
 * `brand` solo llega en la moderación (etiqueta de la marca) y `canParticipate`
 * es false cuando el equipo de la plataforma mira una propuesta de otra marca:
 * la ve, pero su voto o su nombre no deben aparecer en un Lab ajeno.
 */
...
export function LabDetalle({
  ...
  async function vote(kind: LabVoteKind) {
    if (!data || busy || data.canParticipate === false) return;
    setBusy(true);
    try {
      ...
      <div className="grid lg:grid-cols-[1fr,300px] gap-6">
        <div>
          ...
          <div className="card card-pad mb-4">
              ...
              {(Object.keys(VOTE_META) as LabVoteKind[]).map((k) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => vote(k)}
                    disabled={busy || !participa}
                    className={`p-3 rounded-xl border text-center transition ${
                      participa ? 'cursor-pointer' : 'cursor-default'
                    } ${
                      isMine
            ...
            {!participa && (
              <p className="text-xs text-mute2 mt-2 mb-0">
                {/* Sin etiqueta de marca también puede ser solo lectura (sesión
                    suplantada): «otra marca» sería falso. */}
                {data.brand?.name
                  ? `Es una propuesta del Lab de ${data.brand.name}: puedes verla, pero no votar ni comentar.`
                  : 'Puedes ver esta propuesta, pero no votar ni comentar.'}
              </p>
            )}
          </div>
