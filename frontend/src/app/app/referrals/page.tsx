...
const STATUS_LABEL: Record<Use['status'], { text: string; cls: string }> = {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <Kpi label="Inscritos" value={t.signedUp} accent="brand" />
        <Kpi label="Pagando" value={t.converted} accent="ok" />
        <Kpi
          label="Comisión pagada"
          value={`USD ${t.paidUsd.toFixed(2)}`}
          accent="ok"
        />
        <Kpi
          label="Comisión pendiente"
          value={`USD ${t.pendingUsd.toFixed(2)}`}
          accent="amber"
        />
      </div>
      ...
      {codes.length === 0 ? (
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-start justify-between flex-wrap gap-3">
                <div>
                  ...
                  <div className="text-xs text-mute mt-1">
                    ...
                    · {c.commissionPercent}% comisión
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-mute">Inscritos</span>
                    <span className="font-semibold">{c.usesCount}</span>
                    <span className="text-mute">·</span>
                    <span className="text-mute">Pagando</span>
                  ...
                  >
                    <Icon name="check" size={12} /> Copiar link
                  </button>
                </div>
              </div>
              ...
              {c.uses.length > 0 && (
                <details className="mt-3">
                  <summary className="text-xs text-brand cursor-pointer hover:underline">
                    Ver detalle de inscritos ({c.uses.length})
                  </summary>
                  <div className="mt-2 space-y-1.5 text-xs">
                    {c.uses.map((u) => {
                      const meta = STATUS_LABEL[u.status];
                      return (
                        <div
                          key={u.id}
                        ...
                        >
                          <div className="min-w-0 flex-1">
                            <div className="font-medium truncate">
                              {u.tenantBrand ?? 'Tenant pendiente'}
                            </div>
          ...
          <div className="card card-pad text-center">
            <div className="font-semibold mb-1">¿Quieres otro código?</div>
            <p className="text-sm text-mute mb-3">
              Crea códigos adicionales (ej. uno por canal: WhatsApp, IG, eventos).
            </p>
            <Link
              href="/refer"
              target="_blank"
            ...
            >
              <Icon name="spark" /> Generar código nuevo
            </Link>
          </div>
        </div>
      )}
