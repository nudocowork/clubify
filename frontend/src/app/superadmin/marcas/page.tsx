          ...
          <>
            ...
            <div className="p-5 space-y-5">
              ...
              {w.modules.some(
                ...
                <AutomatizacionesConfig whiteLabelId={w.id} onSaved={onChanged} />
              )}
            </div>
...
/**
 * Automatizaciones por marca: mensajes SMS/WhatsApp que el sistema envía,
 * organizados en CARPETAS (Administrativa / Cobros / Operativas + las que crees).
 * Solo se monta si el módulo GROW_BUSINESS_SMS está activo (gating en el
 * llamador). Editar vacío = volver al texto por defecto. Los workflows con
 * estado "pending" son editables pero su envío se activa en un paso posterior.
 */
function AutomatizacionesConfig({
  whiteLabelId,
  onSaved,
}: {
  whiteLabelId: string;
  ...
  async function load() {
    ...
    try {
      apply(
        await api(`/superadmin/white-labels/${whiteLabelId}/message-templates`),
      );
    } catch {
      /* noop */
    } finally {
  ...
  async function save(id: string, text: string) {
    ...
    try {
      apply(
        await api(
          `/superadmin/white-labels/${whiteLabelId}/message-templates/${id}`,
          { method: 'PATCH', body: JSON.stringify({ text }) },
        ),
      );
      onSaved(text.trim() ? 'Mensaje actualizado' : 'Mensaje restaurado');
    } catch (e: any) {
      onSaved(e.message ?? 'Error al guardar el mensaje');
    } finally {
      setSavingId(null);
    }
  }
  ...
  async function moveTo(id: string, folderId: string) {
    ...
    try {
      apply(
        await api(
          `/superadmin/white-labels/${whiteLabelId}/message-templates/${id}/folder`,
          { method: 'PATCH', body: JSON.stringify({ folderId }) },
        ),
      );
    } catch (e: any) {
      onSaved(e.message ?? 'Error al mover');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function createFolder() {
    ...
    try {
      apply(
        await api(`/superadmin/white-labels/${whiteLabelId}/automation-folders`, {
          method: 'POST',
          body: JSON.stringify({ name }),
        }),
      );
      onSaved('Carpeta creada');
    } catch (e: any) {
      onSaved(e.message ?? 'Error al crear carpeta');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function renameFolder(f: BrandMsgFolder) {
    ...
    try {
      apply(
        await api(
          `/superadmin/white-labels/${whiteLabelId}/automation-folders/${f.id}`,
          { method: 'PATCH', body: JSON.stringify({ name }) },
        ),
      );
      onSaved('Carpeta renombrada');
    } catch (e: any) {
      onSaved(e.message ?? 'Error al renombrar');
    } finally {
      setBusy(false);
    }
  }
  ...
  async function deleteFolder(f: BrandMsgFolder) {
    ...
    try {
      ...
      onSaved('Carpeta borrada');
    } catch (e: any) {
      onSaved(e.message ?? 'Error al borrar');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-2">
        <SectionTitle>Automatizaciones (mensajes automáticos)</SectionTitle>
        {!loading && (
          <button
            onClick={createFolder}
            disabled={busy}
            className="text-xs font-semibold rounded-[8px] py-1 px-2.5 shrink-0"
      ...
      >
        Los SMS/WhatsApp que el sistema envía a los negocios de esta marca,
        organizados en carpetas. Personaliza el texto por marca; deja el campo
        vacío y guarda para volver al default. Los tokens <code>{'{token}'}</code>{' '}
        se reemplazan al enviar. Los marcados <b>“Envío por activar”</b> son
        editables pero aún no se envían.
      </div>

      {loading ? (
        <div className="mt-2 text-sm" style={{ color: '#9aa4af' }}>
          Cargando…
        </div>
      ) : (
          ...
          {folders.map((f) => {
            ...
            return (
              <div key={f.id}>
                ...
                {items.length === 0 ? (
                  <div
                    className="text-[11px] mb-1 rounded-lg px-3 py-2"
