'use client';
/**
 * /ambassador/register/<influencerCode> — registro público de embajadores
 * bajo un influencer. Espejo de /seller/register. (2026-06-16)
 *
 * Flow: lookup → form → POST /ambassador/register → setSession → /affiliate.
 */
...
type LookupOk = {
  valid: true;
  influencer: { code: string; ownerName: string; slug: string };
  defaultAmbassadorPercent: number;
};
type LookupFail = {
  valid: false;
  reason: 'INVALID_CODE' | 'NOT_FOUND' | 'INACTIVE';
};
type Lookup = LookupOk | LookupFail;

export default function AmbassadorRegisterClient({ code }: { code: string }) {
  ...
  useEffect(() => {
    let alive = true;
    fetch(`${API}/api/ambassador/register/lookup/${encodeURIComponent(code)}`)
      .then(async (res) => {
        if (!res.ok) return { valid: false, reason: 'NOT_FOUND' } as LookupFail;
        return (await res.json()) as Lookup;
      })
      .then((data) => {
        if (alive) setLookup(data);
      })
      .catch(() => {
        if (alive) setLookup({ valid: false, reason: 'NOT_FOUND' });
      })
      .finally(() => {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`${API}/api/ambassador/register`, {
        method: 'POST',
      ...
      router.push('/affiliate?welcome=ambassador');
    } catch (e: any) {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="card shadow-xl p-6 sm:p-8 text-center">
            ...
            <p className="text-sm text-mute leading-relaxed">
              Este link de registro de embajadores no está activo.
            </p>
            <p className="text-xs text-mute mt-3 leading-snug">
              Pedile a tu influencer que confirme que su link sigue vigente.
            </p>
            <Link href="/" className="inline-block mt-5 btn-ghost text-sm">
              ← Volver al inicio
            </Link>
          </div>
        </div>
  ...
  const influencer = lookup.influencer;
  const pct = lookup.defaultAmbassadorPercent;
        ...
        <div className="card shadow-xl p-6 sm:p-8">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-pill bg-brand-soft text-brand text-xs font-semibold uppercase tracking-wider">
              🤝 Sumate como embajador
            </div>
            <h1 className="mt-3 text-2xl sm:text-3xl font-bold tracking-tight">
              Regístrate como embajador
            </h1>
            <p className="mt-1.5 text-sm text-mute leading-relaxed">
              Te suma a la red de{' '}
              <strong className="text-ink">{influencer.ownerName}</strong>.
              {pct > 0 && (
                <>
                  {' '}Cobras <strong className="text-ink">{pct}%</strong> de
                  comisión por cada negocio que traigas.
                </>
              )}
            </p>
          </div>
          ...
          <form onSubmit={submit} className="mt-6 space-y-3">
            ...
            <div className="text-[11px] text-mute text-center pt-1 leading-snug">
              Al registrarte, te creamos una cuenta de embajador bajo{' '}
              {influencer.ownerName} y entrás directo a tu panel.
            </div>
          </form>
        </div>
