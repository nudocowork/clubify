              ...
              >
                ...
                <div className="p-4">
                      ...
                      <>
                        ...
                        {saved !== null && (
                          <div className="text-[11px] text-bad font-semibold mt-1">
                            {tt('storefront.promo_you_save', {
                              amount: fmt(saved, s.currency, currencySymbol),
                            })}
                          </div>
                        )}
                      </>
                  ...
                  <button
                    ...
                    style={{ background: primary }}
                  >
                    {ordersAllowed ? tt('storefront.promo_select') : tt('storefront.promo_view_more')}
                  </button>
                </div>
              </div>
            );
...
function PromoModal({
  ...
  // deshabilitamos el botón (caso BUY_X_GET_Y / FREE_ITEM / COMBO sin
  // precio referencial). En ese caso ofrecemos "Pedir por WhatsApp" para
  // que el cliente coordine con el negocio.
  ...
  const canWhatsapp = ordersEnabled && !canAddToCart && waDigits.length >= 6;
  const waHref = canWhatsapp
    ? `https://wa.me/${waDigits}?text=${encodeURIComponent(
        `Hola! Quiero pedir esta promoción: ${promo.name}`,
      )}`
    : null;
        ...
        <div className="px-5 py-4 border-t border-line space-y-2 shrink-0">
          {canAddToCart && (
            <button
              ...
              style={{ background: primary }}
            >
              {tt('storefront.add_to_cart', {
                price: fmt(cartUnitPrice, currency, currencySymbol),
              })}
            </button>
          )}
          {waHref && (
            <a
              ...
              onClick={onClose}
              className="w-full py-3 rounded-pill text-white font-semibold shadow-md active:scale-[0.97] transition-transform flex items-center justify-center gap-2"
              style={{ background: '#25D366' }}
            >
              <svg
                ...
                <div>
                  ...
                  {regionsData.regions.length > 0 && munList.length > 0 ? (
                    ...
                    >
                      ...
                      {form.departamento && (
                        <option value={OTRO_MUNICIPIO}>{tt('checkout.city_other')}</option>
                      )}
                    </select>
                  ) : (
                    <input
                      ...
                      <>
                        <label className="label">
                          {matchingSedes.length
                            ? tt('checkout.confirm_location')
                            : tt('checkout.no_location_state')}
                        </label>
                        <select
                          className="input"
