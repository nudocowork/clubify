// Aplica la migración 20260804_white_label_notify a prod de forma idempotente
...
(async () => {
  ...
  const name = '20260804_white_label_notify';
  ...
  const cols = await prisma.$queryRawUnsafe(
    `SELECT column_name FROM information_schema.columns WHERE table_name='WhiteLabel' AND column_name IN ('notifyPhone','lowCreditsNotifiedAt','pendingClientsNotifiedAt') ORDER BY column_name`,
  );
  console.log(
    '• Columnas notify presentes:',
