// Aplica 20260829_add_data_policy (ADD COLUMN IF NOT EXISTS) y la registra en
// _prisma_migrations. Correr ANTES de deployar el backend.
// Usage: railway run --service Postgres-Nq8w node scripts/apply-data-policy-migration.cjs
...
(async () => {
  ...
  const name = '20260829_add_data_policy';
  ...
  const checks = [
    ['Tenant', 'dataPolicyUrl'],
    ['Card', 'dataPolicyEnabled'],
    ['Pass', 'dataPolicyAcceptedAt'],
    ['Pass', 'dataPolicyUrl'],
  ];
  for (const [table, col] of checks) {
    ...
    console.log(`• ${table}.${col}:`, r.length ? '✓' : 'FALTA ✗');
  }
