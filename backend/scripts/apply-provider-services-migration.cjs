// Aplica 20260822_service_provider_services (idempotente): ServiceProvider.serviceIds.
// Usage: railway run --service Postgres-Nq8w node backend/scripts/apply-provider-services-migration.cjs
...
(async () => {
  ...
  const name = '20260822_service_provider_services';

  const col = await prisma.$queryRawUnsafe(
    `SELECT 1 FROM information_schema.columns WHERE table_name='ServiceProvider' AND column_name='serviceIds' LIMIT 1`,
  );
  if (col.length) {
    console.log('• Columna serviceIds ya existe — salto el DDL.');
  } else {
