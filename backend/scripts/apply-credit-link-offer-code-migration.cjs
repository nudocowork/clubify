// Apply migration 20260813_credit_link_offer_code to prod Postgres-Nq8w.
// Agrega HotmartCreditLink.hotmartOfferCode (distinguir packs 1/10/20 que
...
const MIGRATION = '20260813_credit_link_offer_code';
...
(async () => {
  ...
  if (!exists.length) {
    await prisma.$executeRawUnsafe(
      `INSERT INTO _prisma_migrations
       ...
       VALUES (gen_random_uuid()::text, 'manual-apply-offer-code', NOW(), '${MIGRATION}', 'applied via script', NULL, NOW(), 1)`,
    );
  ...
  const col = await prisma.$queryRawUnsafe(
    `SELECT column_name FROM information_schema.columns
      WHERE table_name = 'HotmartCreditLink' AND column_name = 'hotmartOfferCode'`,
  );
  console.log(col.length ? '✅ Columna hotmartOfferCode presente.' : '❌ Columna NO encontrada.');
