...
// ── Carpetas de plantillas de correo (árbol anidado) ────────────────────────
// Mismo enfoque que brand-workflow-folders.service.ts, adaptado a las tablas
// de plantillas. El árbol se arma en código a partir de `parentId` (sin
// @relation en el schema, a propósito: una FK con cascada borraría subárboles
// enteros en silencio al eliminar una carpeta padre). Este servicio es el
// ÚNICO punto que muta el árbol, y concentra las dos invariantes que evitan
// pérdida de datos:
...
//   2. Borrar una carpeta NUNCA borra su contenido: plantillas y subcarpetas
...
@Injectable()
export class MktTemplateFoldersService {
  ...
  async create(whiteLabelId: string, name: string, parentId?: string | null) {
    ...
    const position = await this.prisma.mktEmailTemplateFolder.count({ where: { whiteLabelId } });
    return this.prisma.mktEmailTemplateFolder.create({
  ...
  async rename(whiteLabelId: string, folderId: string, name: string) {
    ...
    await this.prisma.mktEmailTemplateFolder.update({
      where: { id: folderId },
  ...
  async move(whiteLabelId: string, folderId: string, parentId: string | null) {
    ...
    if (target) {
      ...
      const all = await this.prisma.mktEmailTemplateFolder.findMany({
        where: { whiteLabelId },
  ...
  // Borra la carpeta pero NUNCA su contenido: plantillas y subcarpetas suben
  // al padre de la carpeta borrada (raíz si estaba en la raíz). En
  // transacción: si algo falla a mitad, no queda contenido colgando de una
  // carpeta que ya no existe (= invisible en la UI).
  async remove(whiteLabelId: string, folderId: string) {
    ...
    const [tpls, subs] = await this.prisma.$transaction([
      this.prisma.mktEmailTemplate.updateMany({
        where: { whiteLabelId, folderId },
        data: { folderId: newParent },
      }),
      this.prisma.mktEmailTemplateFolder.updateMany({
        where: { whiteLabelId, parentId: folderId },
        data: { parentId: newParent },
      }),
      this.prisma.mktEmailTemplateFolder.delete({ where: { id: folderId } }),
    ]);
    return { ok: true, movedTemplates: tpls.count, movedFolders: subs.count };
  }
}
